...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len > storage.len() {
            panic!("Tried to initialize vec with len greater than backing store")
        }
        StackVec {
            storage,
            len
...
impl<'a, T: 'a> core::ops::Deref for StackVec<'a, T> {
    type Target = [T];
...
impl<'a, T: 'a> core::ops::DerefMut for StackVec<'a, T> {
