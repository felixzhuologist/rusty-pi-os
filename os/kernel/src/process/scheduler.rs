...
impl GlobalScheduler {
    ...
    pub fn start(&self) {
        ...
        first_process.trap_frame.elr = start_shell as u64;

        unsafe {
            // move the stack pointer to the first process' trap frame
            asm!("mov SP, $0" :: "r"(&*first_process.trap_frame) :: "volatile");

            // call context restore to set registers
            asm!("bl context_restore" :::: "volatile");

            // set the current stack pointer to an initial value of start and then
            // reset register values back to 0
            asm!("ldr x1, = _start
                 mov SP, x1
                 mov x1, #0"
                 :::: "volatile");
