...
impl Xmodem<()> {
        ...
        where W: io::Read + io::Write, R: io::Read
    {
        ...
        (transmitter.progress)(Progress::Waiting);
        transmitter.expect_byte(NAK, "Did not get NAK to initiate send")?;
        (transmitter.progress)(Progress::Started);
       ...
       where R: io::Read + io::Write, W: io::Write
    {
        ...
        (receiver.progress)(Progress::Started);
...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        ...
        let read = self.read_byte(abort_on_can)?;
        if read == byte {
            Ok(read)
        } else {
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if first == EOT {
            self.write_byte(NAK)?;
            self.expect_byte_or_cancel(EOT, "Expected second EOT byte")?;
            self.write_byte(ACK)?;
            return Ok(0)
        } else if first != SOH {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Expected EOT or SOH"))
        }

        self.expect_byte_or_cancel(self.packet, "Expected different packet number")?;
        self.expect_byte_or_cancel(!self.packet, "Expected different 1's complement")?;
        ...
        if self.read_byte(false)? != checksum {
            self.write_byte(NAK)?;
            return Err(io::Error::new(io::ErrorKind::Interrupted, "Incorrect checksum"));
        }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        match self.read_byte(true)? {
            NAK => Err(io::Error::new(io::ErrorKind::Interrupted, "Got NAK")),
            ACK => Ok(buf.len()),
